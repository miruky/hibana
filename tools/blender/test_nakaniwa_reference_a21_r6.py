...
from tools.blender.stage_kits import nakaniwa_reference_a21_r6 as NAKANIWA
...
class NakaniwaReferenceA21Tests(unittest.TestCase):
    ...
    def test_palace_is_tall_layered_arcaded_and_heavily_crowned(self):
        ...
        crown_petal_bounds = [
            NAKANIWA.spec_bounds(spec) for spec in crown_petals
        ]
        self.assertTrue(
            all(bound[4] - bound[1] >= 12.5 for bound in crown_petal_bounds)
        )
        self.assertTrue(
            all(
                max(bound[3] - bound[0], bound[5] - bound[2]) >= 12.0
                for bound in crown_petal_bounds
            )
        )
        crown_support_bounds = [
            NAKANIWA.spec_bounds(spec)
            for spec in specs
            if spec["role"].startswith(
                (
                    "a21-r5-palace-rooted-crown-keep-shoulder",
        ...
        crown_support_top = max(bound[4] for bound in crown_support_bounds)
        self.assertGreaterEqual(crown_support_top, 33.9)
        self.assertTrue(
            all(bound[1] <= crown_support_top for bound in crown_petal_bounds)
        )
        self.assertGreaterEqual(
            max(bound[4] for bound in crown_petal_bounds),
            42.8,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            role_count(
                specs,
                "a21-r6-palace-vertical-tower-deep-occupied-window"
                "-recessed-glazing",
            ),
            8,
        )
        ...
        self.assertFalse(
            any(
                any(
                    spec["role"].startswith(prefix)
                    for prefix in NAKANIWA.R6_VISUAL_BLOCKER_PREFIXES
                )
                for spec in specs
            )
        )
        ...
        self.assertGreaterEqual(max(bound[4] for bound in palace_bounds), 42.8)
    ...
    def test_near_mid_far_density_boundary_and_story_are_real_geometry(self):
        ...
        self.assertEqual(
            role_count(specs, "a21-r6-bridge-axis-weathered-paving-slab"),
            11,
        )
        self.assertEqual(
            role_count(specs, "a21-r6-bridge-axis-shallow-water-rill"),
            2,
        )
        self.assertEqual(
            role_count(specs, "a21-r6-edge-garden-low-chamfered-planter"),
            4,
        )
        self.assertEqual(
            role_count(specs, "a21-r6-edge-garden-pruned-tree-trunk"),
            1,
        )
        self.assertEqual(
            role_count(specs, "a21-r6-bridge-edge-warm-lantern"),
            2,
        )
        self.assertEqual(
            role_count(specs, "a21-r6-edge-story-gardener-bench"),
            1,
        )
        self.assertEqual(
            role_count(
        ...
        self.assertEqual(
            role_count(
                specs,
                "a21-r6-foreground-edge-small-flowering-tree-trunk",
            ),
            2,
        )
        self.assertEqual(
    ...
    def test_locked_player_height_camera_shows_both_heroes_large(self):
        ...
        self.assertGreaterEqual(camera["lensMm"], 23.0)
        self.assertLessEqual(camera["lensMm"], 26.0)
        ...
        for hero in metrics["heroes"]:
            ...
            self.assertGreaterEqual(hero["visibleFrameHeightRatio"], 0.30)
            self.assertTrue(hero["fullyFramed"], hero)
        vaults = NAKANIWA.conservatory_five_vault_frame_report(0)
        self.assertTrue(vaults["passed"], vaults)
        self.assertEqual(len(vaults["vaults"]), 5)
        self.assertTrue(all(vault["fullyFramed"] for vault in vaults["vaults"]))
        self.assertEqual(vaults["legacyOpaqueBlockerCount"], 0)
        occlusion = NAKANIWA.reference_camera_occlusion_report(0)
        self.assertTrue(occlusion["passed"], occlusion)
        self.assertTrue(
            all(hero["occlusionRatio"] <= 0.18 for hero in occlusion["heroes"])
        )
        foreground = NAKANIWA.r6_foreground_occupancy_report(0)
        self.assertTrue(foreground["passed"], foreground)
        self.assertLessEqual(foreground["centralAreaRatio"], 0.06)
        self.assertEqual(foreground["forbiddenRoleCount"], 0)
    ...
    def test_materials_are_shared_pbr_and_realistically_bounded(self):
        ...
        self.assertLessEqual(max(ivory[:3]) - min(ivory[:3]), 0.20)
        self.assertLessEqual(max(carved[:3]) - min(carved[:3]), 0.14)
    ...
    def test_output_and_scorecard_remain_private_no_ship(self):
        ...
        self.assertTrue(str(NAKANIWA.PRIVATE_PRODUCTION_DEFAULT).endswith("-r6"))
        ...
        self.assertEqual(
            NAKANIWA.SELF_REJECT_HISTORY[-2]["verdict"],
            "INDEPENDENT_REJECTED_R6_VERTICAL_PALACE_OCCUPANCY",
        )
        self.assertEqual(
            NAKANIWA.SELF_REJECT_HISTORY[-1]["verdict"],
            "INDEPENDENT_REJECTED_R6_VERTICAL_AMPLITUDE_INSUFFICIENT",
        )
    ...
    @unittest.skipUnless(
        ...
        and NAKANIWA.R5_SOURCE_PATH.is_file()
        and NAKANIWA.R5_TEST_PATH.is_file()
        and NAKANIWA.R5_CANDIDATE_PATH.is_file()
        and NAKANIWA.R5_MANIFEST_PATH.is_file(),
        "immutable private R3/R4/R5 evidence is unavailable",
    )
    def test_independent_source_evidence_is_hash_locked_and_untouched(self):
        ...
        r5_report = NAKANIWA.locked_r5_report()
        self.assertTrue(r5_report["matched"], r5_report)
        self.assertFalse(r5_report["writeAttempted"])
        self.assertEqual(len(r5_report["artifacts"]), 4)
        for artifact in r5_report["artifacts"]:
            self.assertTrue(artifact["matched"], artifact)
            self.assertEqual(
                hashlib.sha256(Path(artifact["path"]).read_bytes()).hexdigest(),
                artifact["expectedSha256"],
