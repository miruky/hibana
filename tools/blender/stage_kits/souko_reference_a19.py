#!/usr/bin/env python3
"""Independent Souko A19 macro-rebuild plan.
...
------------------------------
* Fix the primary shot first: a 1.65 m eye on the north-west quay, 32 mm lens,
...
STAGE_ID = "souko"
REFERENCE_MATCH_VERSION = "a19-souko-macro-rebuild-v1"
...
PRIVATE_OUTPUT_ROOT = Path("/private/tmp/hibana-blender/a19-souko-macro-v1")
TARGET_COLLECTION = "HB_SOUKO_A19_PRIVATE"
MAP_SIZE_M = 336.0
PLAYER_EYE_M = 1.65
...
# The first shot is a non-negotiable A19 contract, not a render-time guess.
PRIMARY_CAMERA: dict[str, Any] = {
    "id": "01-a19-dual-hero-quay",
    "eye": (-240.0, PLAYER_EYE_M, 170.0),
    "target": (5.0, 38.0, 14.0),
    "lensMm": 32.0,
    "sensorWidthMm": 36.0,
    "frameOrder": (STACKHOUSE_ID, CUSTOMS_ID),
    "skyMaxFraction": 0.25,
    "roadMaxFraction": 0.30,
    "heroHorizontalFillTarget": (0.70, 0.88),
    "purpose": "Tight ImageGen-like dual-hero identity view from occupied north-west quay.",
}
...
PRIVATE_VIEWS: tuple[dict[str, Any], ...] = (
    PRIMARY_CAMERA,
    {
        ...
        "eye": (12.0, PLAYER_EYE_M, 39.0),
        "target": (82.0, 34.0, 96.0),
        "lensMm": 30.0,
        "purpose": "Occupied four-tower silhouette and bridge arrival.",
    },
    {
        "id": "03-stackhouse-rack-interior",
        "eye": (8.0, PLAYER_EYE_M, 96.0),
        "target": (95.0, 7.5, 96.0),
        "lensMm": 30.0,
        "purpose": "Diagonal deep rack floors, cargo, piers and bridge underside.",
    },
    {
        "id": "04-stackhouse-east-crown",
        "eye": (165.0, PLAYER_EYE_M, 28.0),
        "target": (84.0, 58.0, 96.0),
        "lensMm": 32.0,
        "purpose": "South-east crown, bridge depth, roof plant and rust hierarchy.",
    },
    {
        "id": "05-customs-loading-approach",
        "eye": (-68.0, PLAYER_EYE_M, -175.0),
        "target": (-68.0, 35.0, -67.8),
        "lensMm": 32.0,
        "purpose": "Loading base, four glazed teeth, control tower and chimneys.",
    },
    {
        "id": "06-customs-sawtooth-depth",
        "eye": (-5.0, PLAYER_EYE_M, -185.0),
        "target": (-66.0, 35.0, -68.0),
        "lensMm": 32.0,
        "purpose": "Four full-depth sawtooth planes, trusses and occupied bays.",
    },
    {
        "id": "07-loading-life",
        "eye": (-210.0, PLAYER_EYE_M, 176.0),
        "target": (-111.0, 4.5, 153.0),
        "lensMm": 35.0,
        "purpose": "Occupied shed face, pallets, containers, forklift, rail and wet curb.",
    },
    {
        "id": "08-quay-ship-cranes",
        "eye": (70.0, PLAYER_EYE_M, 260.0),
        "target": (-45.0, 11.0, 188.0),
        "lensMm": 30.0,
...
LOD_API = {
    0: {"label": "hero", "maxSpecs": 3600, "maxEstimatedTriangles": 240_000},
    1: {"label": "medium", "maxSpecs": 2100, "maxEstimatedTriangles": 120_000},
    2: {"label": "horizon", "maxSpecs": 1050, "maxEstimatedTriangles": 52_000},
}

MATERIALS: dict[str, dict[str, Any]] = {
    "wet_asphalt": {
        "color": (0.025, 0.032, 0.034, 1.0), "roughness": 0.24, "metallic": 0.0,
        "noise": 0.23, "wetVariation": True,
    },
    "puddle_water": {
    ...
    "old_concrete": {
        "color": (0.34, 0.33, 0.30, 1.0), "roughness": 0.86,
        "metallic": 0.0, "noise": 0.22, "stains": True,
    },
    "pale_concrete": {
        "color": (0.56, 0.54, 0.48, 1.0), "roughness": 0.76,
        "metallic": 0.0, "noise": 0.16, "stains": True,
    },
    "dark_concrete": {
        "color": (0.14, 0.16, 0.16, 1.0), "roughness": 0.80,
        "metallic": 0.0, "noise": 0.19, "stains": True,
    },
    "weathered_zinc": {
        "color": (0.29, 0.35, 0.36, 1.0), "roughness": 0.43,
        "metallic": 0.70, "noise": 0.16, "rustMask": True,
    },
    "structural_steel": {
        "color": (0.085, 0.105, 0.11, 1.0), "roughness": 0.48,
        "metallic": 0.88, "noise": 0.11, "rustMask": True,
    },
    "red_brick": {
        "color": (0.28, 0.070, 0.040, 1.0), "roughness": 0.86,
        "metallic": 0.0, "noise": 0.20, "stains": True,
    },
    "rust": {
        "color": (0.38, 0.075, 0.018, 1.0), "roughness": 0.87,
        "metallic": 0.02, "noise": 0.24,
    },
    "safety_orange": {
        "color": (0.72, 0.16, 0.025, 1.0), "roughness": 0.46,
        "metallic": 0.0, "noise": 0.07,
    },
    "dirty_glass": {
        "color": (0.065, 0.14, 0.17, 0.62), "roughness": 0.20,
        "metallic": 0.04, "transmission": 0.52, "alpha": 0.62, "noise": 0.07,
    },
    "warm_glass": {
        "color": (0.30, 0.12, 0.035, 1.0), "roughness": 0.28,
        "metallic": 0.0, "emission": (0.36, 0.095, 0.014, 1.0),
...
def _build_ground_and_foreground(plan: SpecPlan, lod: int) -> None:
    ...
    shed_group = "souko-a19-camera-loading-shed"
    ...
    cluster_points = (
        (-88.0, 166.5), (-97.0, 169.0), (-107.0, 167.5),
        (-123.0, 169.0), (-136.0, 164.0), (-142.0, 155.0),
...
def _add_stackhouse_tower(
    ...
    lower_floor_names = []
    lower_levels = (5.5, 10.8, 16.1) if lod == 0 else (7.0, 16.0) if lod == 1 else (16.0,)
...
def _build_inter_landmark_transfer(plan: SpecPlan, lod: int) -> None:
    ...
    group = "souko-a19-inter-landmark-transfer"
    start = (33.0, 77.0)
    end = (-23.0, -31.0)
...
def _build_layered_city(plan: SpecPlan, lod: int) -> None:
    ...
    horizon_group = "souko-a19-central-process-horizon"
    silo_specs = (
        (42.0, -116.0, 9.0, 72.0),
...
def build_plan(lod: int = 0) -> SpecPlan:
    """Return the deterministic standalone A19 Souko plan."""
    plan = SpecPlan(lod)
    ...
    items = [
        {
            "category": category,
            "score": 0.0,
            "evidence": "A19 producer build; independent visual review has not certified this category.",
        }
