#!/usr/bin/env python3
"""Private A21 R5 production-art rebuild for Nakaniwa.
...
``/private/tmp/hibana-blender/a21-nakaniwa-production-art-r5``.  It never
edits public assets, runtime source, manifests, A20, the immutable R3 proof,
the immutable R4 source/evidence, or a visible Blender session.  The
...
STAGE_ID = "nakaniwa"
KIT_VERSION = "nakaniwa-reference-a21-production-r5"
...
MATERIALS = {
    "wet_stone": {
        "color": (0.070, 0.078, 0.073, 1.0),
        "roughness": (0.10, 0.30), "metallic": 0.03,
        "noiseScale": 1.15, "detailScale": 30.0, "bump": 0.065,
    },
    "ivory_stone": {
        "color": (0.76, 0.735, 0.675, 1.0),
        "roughness": (0.22, 0.44), "metallic": 0.0,
        "noiseScale": 0.42, "detailScale": 28.0, "bump": 0.060,
    },
    "carved_stone": {
        "color": (0.50, 0.475, 0.445, 1.0),
        "roughness": (0.25, 0.50), "metallic": 0.0,
        "noiseScale": 0.88, "detailScale": 36.0, "bump": 0.055,
    },
    "moss_stone": {
    ...
    "water": {
        "color": (0.008, 0.075, 0.095, 1.0),
        "roughness": (0.035, 0.14), "metallic": 0.0,
        "transmission": 0.25, "alpha": 1.0, "ior": 1.333,
        "noiseScale": 0.72, "detailScale": 40.0, "bump": 0.040,
    },
    "dark_wood": {
...
MAIN_REFERENCE_CAMERA = {
    ...
    "location": (74.46, PLAYER_EYE_M, -90.91),
    "target": (-5.0, 14.0, -4.0),
    "lensMm": 20.0,
    "sensorWidthMm": 36.0,
    "resolution": (1280, 720),
    "eyeHeightM": PLAYER_EYE_M,
    "intent": "bridge-first canal garden with both complete monumental heroes",
}
...
def _add_r5_palace_system(specs: list[dict], lod: int) -> None:
    ...
    face_x = root_x + normal_x * 13.0
    face_z = root_z + normal_z * 13.0
    root_loggia_bays = (5, 4, 3)[lod]
    ...
    petal_offsets = (
        ...
        if lod == 0
        else (-8.0, -4.0, 0.0, 4.0, 8.0)
        if lod == 1
        else (-6.0, 0.0, 6.0)
    )
    petal_plane_x = root_x + normal_x * 9.8
    petal_plane_z = root_z + normal_z * 9.8
    ...
    for petal_index, offset in enumerate(petal_offsets):
        ...
        tip_centre = offset * 1.04
        root_y = 29.1
        waist_y = 35.0 - abs(offset) * 0.025
        shoulder_y = 39.1 - abs(offset) * 0.055
        tip_y = 42.52 - abs(offset) * 0.065
        root_half = (3.55, 3.15, 2.70)[lod]
        waist_half = (3.00, 2.65, 2.25)[lod]
        shoulder_half = (2.18, 1.90, 1.60)[lod]
        petal_corners = (
        ...
        _sweep(
            ...
            (
                ...
                petal_point(tip_centre, min(42.88, tip_y + 0.32), 0.52),
            ),
            (0.075, 0.060, 0.046)[lod],
            (8, 6, 4)[lod],
        )
        if lod <= 1:
            inset = tuple(
                (
                    point[0] * 0.94 + petal_plane_x * 0.06
                    + normal_x * 0.52,
                    point[1] * 0.96 + 35.5 * 0.04,
                    point[2] * 0.94 + petal_plane_z * 0.06
                    + normal_z * 0.52,
                )
                for point in petal_corners
            )
            _panel(
                specs,
                "a21-r5-palace-petal-carved-inner-relief",
                "flower" if petal_index in {2, 4} else "carved_stone",
                group,
                inset,
                0.10,
            )
        if lod == 0:
            for edge_points in (
                (petal_corners[0], petal_corners[6], petal_corners[5],
                 petal_corners[4]),
                (petal_corners[1], petal_corners[2], petal_corners[3],
                 petal_corners[4]),
            ):
                _sweep(
                    specs,
...
def _add_r5_garden_city_depth(specs: list[dict], lod: int) -> None:
    ...
    sites = (
        (-116.0, 12.0, 24.0, 18.0, 26.0),
        (-89.0, 34.0, 22.0, 17.0, 29.0),
        (-60.0, 50.0, 23.0, 18.0, 25.0),
        (-29.0, 65.0, 24.0, 18.0, 30.0),
        (0.0, 80.0, 21.0, 17.0, 24.0),
        (25.0, 105.0, 24.0, 18.0, 27.0),
        (53.0, 118.0, 20.0, 17.0, 26.0),
        (82.0, 112.0, 22.0, 18.0, 29.0),
        (107.0, 95.0, 23.0, 19.0, 25.0),
        (122.0, 64.0, 22.0, 18.0, 28.0),
    )
...
def independent_baseline_scorecard(evidence_paths: Sequence[str] = ()) -> dict:
    ...
    return {
        ...
        "strongestRemainingMismatch": (
            ...
            "reviewer compares the R5 candidate at original resolution."
        ),
...
def _add_world_and_lights(lighting):
    ...
    sky.sun_rotation = math.radians(232.0)
    sky.air_density = 1.18
    ...
    sun_data.energy = 4.10
    sun_data.angle = math.radians(1.05)
    sun_data.color = (1.0, 0.68, 0.42)
    ...
    fill_data.size = 78.0
    fill_data.color = (0.42, 0.56, 0.72)
    ...
    bounce_data.energy = 230.0
    bounce_data.shape = "DISK"
    bounce_data.size = 64.0
    bounce_data.color = (1.0, 0.62, 0.31)
