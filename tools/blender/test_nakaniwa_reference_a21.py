...
from tools.blender.stage_kits import nakaniwa_reference_a21 as NAKANIWA
...
class NakaniwaReferenceA21Tests(unittest.TestCase):
    ...
    def test_palace_is_tall_layered_arcaded_and_heavily_crowned(self):
        ...
        self.assertEqual(
            role_count(specs, "a21-palace-companion-tower-carved-belt"), 6
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-palace-central-keep-weathered-stringcourse",
            ),
            2,
        )
        self.assertGreaterEqual(
            role_count(
                specs,
                "a21-palace-companion-tower-deep-window-recessed-glazing",
            ),
            18,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-palace-central-keep-front-engaged-buttress",
            ),
            6,
        )
        self.assertEqual(
            role_count(specs, "a21-palace-central-keep-upper-turret"),
            2,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-palace-central-keep-upper-turret-window-shadow-recess",
            ),
            2,
        )
        self.assertEqual(
            role_count(specs, "a21-palace-central-keep-carved-relief-panel"),
            5,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-palace-central-keep-upper-side-deep-window-shadow-recess",
            ),
            6,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-palace-central-keep-side-stone-balcony",
            ),
            2,
        )
        self.assertEqual(
            role_count(specs, "a21-palace-central-keep-stone-balcony"), 2
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-palace-lower-water-loggia-deep-occupied-arcade",
            ),
            8,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-palace-upper-garden-loggia-deep-occupied-arcade",
            ),
            6,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-palace-lower-water-loggia-grounded-carved-column",
            ),
            9,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-palace-upper-garden-loggia-grounded-carved-column",
            ),
            7,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-palace-rooted-crown-occupied-loggia",
            ),
            5,
        )
        self.assertEqual(
    ...
    def test_conservatory_is_exactly_five_large_overlapping_vaults(self):
        ...
        self.assertEqual(
            role_count(
                specs,
                "a21-conservatory-entry-drum-weathered-course",
            ),
            4,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-conservatory-entry-drum-vertical-stone-joint",
            ),
            9,
        )
        self.assertEqual(
            role_count(specs, "a21-conservatory-monumental-entry-fan"), 5
        )
        self.assertEqual(
            role_count(specs, "a21-conservatory-monumental-entry-drum"), 1
        )
        self.assertEqual(
            role_count(specs, "a21-conservatory-occupied-glass-lantern"), 1
        )
    ...
    def test_near_mid_far_density_boundary_and_story_are_real_geometry(self):
        ...
        self.assertEqual(
            role_count(specs, "a21-r4-foreground-loggia-grounded-hall"),
            1,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-foreground-loggia-deep-front-arcade-curved-rib",
            ),
            4,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-foreground-loggia-deep-side-arcade",
            ),
            5,
        )
        self.assertEqual(
            role_count(
                specs,
                "a21-r4-foreground-loggia-upper-deep-window-recessed-glazing",
            ),
            5,
        )
        self.assertEqual(
    ...
    def test_materials_are_shared_pbr_and_realistically_bounded(self):
        ...
        self.assertGreater(
            NAKANIWA.MATERIALS["dirty_glass"]["alpha"], 0.30
        )
        self.assertLess(
            NAKANIWA.MATERIALS["dirty_glass"]["alpha"], 0.60
        )
        self.assertGreaterEqual(
    ...
    def test_output_and_scorecard_remain_private_no_ship(self):
        ...
        self.assertEqual(scorecard["arithmeticMean"], 4.50)
        self.assertEqual(scorecard["minimumCategoryScore"], 2.9)
        self.assertEqual(
            scorecard["sourceScorecard"],
            str(NAKANIWA.INDEPENDENT_SCORECARD_R3_PATH),
        )
        self.assertEqual(
        ...
        self.assertTrue(str(NAKANIWA.PRIVATE_PRODUCTION_DEFAULT).endswith("-r4"))
    ...
    @unittest.skipUnless(
        ...
        "immutable private R3 scorecard is unavailable",
    )
    def test_independent_r3_scorecard_is_hash_locked_and_untouched(self):
